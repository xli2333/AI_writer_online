...
const COMMERCIAL_HUMANIZER_RULES = [
...
const COMMERCIAL_HUMANIZER_PATTERN_GROUPS = [
...
const COMMERCIAL_HUMANIZER_QUICK_CHECKS = [
  '连续三个句子长度和结构是否过于整齐。',
...
const buildCommercialHumanizerBlock = () =>
  ['商业文章去AI化护栏：', ...COMMERCIAL_HUMANIZER_RULES.map((rule, index) => `${index + 1}. ${rule}`)].join('\n');

const buildCommercialHumanizerPatternBlock = () =>
  ['商业文章去AI化检查维度：', ...COMMERCIAL_HUMANIZER_PATTERN_GROUPS.map((rule, index) => `${index + 1}. ${rule}`)].join('\n');

const buildCommercialHumanizerChecklistBlock = () =>
  ['商业文章去AI化快速检查：', ...COMMERCIAL_HUMANIZER_QUICK_CHECKS.map((rule, index) => `${index + 1}. ${rule}`)].join('\n');
...
const getExpectedChunkCount = (options: WritingTaskOptions) =>
  Math.max(1, Math.ceil(options.desiredLength / Math.max(600, options.chunkLength)));

const buildChunkTargetLengths = (desiredLength: number, chunkCount: number) => {
  if (chunkCount <= 1) {
...
const buildFallbackChunkPlan = (
  outline: string,
  options: WritingTaskOptions,
  expectedChunkCount = getExpectedChunkCount(options)
): WritingChunkPlanItem[] => {
  const outlineSections = extractOutlineHeadings(outline);
  const groupedSections = distributeOutlineSections(outlineSections, expectedChunkCount);
  const targetLengths = buildChunkTargetLengths(options.desiredLength, expectedChunkCount);
...
const normalizeChunkPlanStrict = (raw: unknown, outline: string, options: WritingTaskOptions): WritingChunkPlanItem[] => {
  const expectedChunkCount = getExpectedChunkCount(options);
...
const buildChunkPlan = async (
  ...
  const expectedChunks = getExpectedChunkCount(options);
  const prompt = [
    buildTaskBrief(topic, direction, options),
