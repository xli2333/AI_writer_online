...
import { getRuntimeSubPersonaDescriptor, resolveRuntimeSubPersona } from './services/stylePersonaUtils';

const fallbackStyleProfiles: StyleProfileDescriptor[] = [
  {
    id: 'fdsm',
    label: '复旦商业文章',
    shortLabel: 'FDSM',
...
const formatTimestampLabel = (value?: string) => {
  ...
  if (Number.isNaN(timestamp.getTime())) return '未记录';
...
const formatWinRateLabel = (value?: number) => (typeof value === 'number' ? `${Math.round(value * 100)}%` : '未记录');
...
const normalizeTaskOptions = (options?: Partial<WritingTaskOptions> | null): WritingTaskOptions => ({
  ...
  ...options,
  styleProfile:
    typeof options?.styleProfile === 'string' && options.styleProfile.trim()
      ? options.styleProfile
      : defaultOptions.styleProfile,
...
const hasCustomizedOptions = (options: WritingTaskOptions) =>
  options.styleProfile !== defaultOptions.styleProfile ||
...
const App: React.FC = () => {
  ...
  const [styleProfiles, setStyleProfiles] = useState<StyleProfileDescriptor[]>(fallbackStyleProfiles);
  const [personaStatuses, setPersonaStatuses] = useState<Record<string, PersonaStatusDescriptor>>({});
  ...
  const activeStyleProfile =
    styleProfiles.find((profile) => profile.id === taskOptions.styleProfile) ||
    fallbackStyleProfiles.find((profile) => profile.id === taskOptions.styleProfile) ||
  ...
  useEffect(() => {
    ...
    loadStyleProfiles()
      .then((profiles) => {
        const resolvedProfiles = Array.isArray(profiles) && profiles.length > 0 ? profiles : fallbackStyleProfiles;
        if (!isMounted) return;
        setStyleProfiles(resolvedProfiles);
        return Promise.all(
          resolvedProfiles.map(async (profile) => [profile.id, await loadPersonaStatus(profile.id)] as const)
        ).then((entries) => {
          if (isMounted) {
            setPersonaStatuses(Object.fromEntries(entries));
          }
        });
      })
      .catch((error) => {
        console.warn('[style-profiles] fallback to local defaults', error);
        if (!isMounted) return;
        void Promise.all(
          fallbackStyleProfiles.map(async (profile) => [profile.id, await loadPersonaStatus(profile.id)] as const)
        )
          .then((entries) => {
            if (isMounted) {
              setPersonaStatuses(Object.fromEntries(entries));
          ...
          .catch((statusError) => {
            console.warn('[persona-status] fallback to empty state', statusError);
          });
      });
              ...
              <form onSubmit={handleStartResearch} className="flex flex-col">
                ...
                <div className="grid gap-4 border-t border-gray-100 bg-gray-50/50 px-5 py-5 text-left md:grid-cols-2 xl:grid-cols-4">
                  <label className="block">
                    <span className="mb-2 block text-xs font-bold uppercase tracking-wider text-slate-400">风格库</span>
                    <select
                      value={taskOptions.styleProfile}
                      onChange={(event) => handleTaskOptionChange('styleProfile', event.target.value)}
                  ...
                  <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 md:col-span-2 xl:col-span-3">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-sm font-semibold text-slate-800">{activeStyleProfile?.label || '当前风格库'}</span>
                      <span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-500">
                        人格版本 {activePersonaStatus?.versionLabel || '加载中'}
                      </span>
                      {activeSubPersona && (
                        <span className="rounded-full bg-teal-50 px-2 py-1 text-[11px] font-medium text-teal-700">
                          当前稿型 {activeSubPersona.label}
                        </span>
                      )}
                    </div>
                    <p className="mt-2 text-xs leading-5 text-slate-500">
                      {activeStyleProfile?.description || '当前风格库描述暂不可用。'}
                    </p>
                    <div className="mt-3 grid gap-2 text-xs text-slate-500 md:grid-cols-2 xl:grid-cols-4">
                      <div className="rounded-lg bg-slate-50 px-3 py-2">
                        <div className="font-medium text-slate-700">主人格更新</div>
                        <div>{formatTimestampLabel(activePersonaStatus?.personaUpdatedAt)}</div>
                        <div>样本数 {activePersonaStatus?.personaSourceCount ?? 0}</div>
                      </div>
                      <div className="rounded-lg bg-slate-50 px-3 py-2">
                        <div className="font-medium text-slate-700">最近进化</div>
                        <div>{formatTimestampLabel(activePersonaStatus?.lastEvolutionAt)}</div>
                        <div>胜率 {formatWinRateLabel(activePersonaStatus?.lastPatchWinRate)}</div>
                      </div>
                      <div className="rounded-lg bg-slate-50 px-3 py-2">
                        <div className="font-medium text-slate-700">基准任务</div>
                        <div>{activePersonaStatus?.benchmarkTaskCount ?? 0} 题</div>
                        <div>反模式 {activePersonaStatus?.antiPatternCount ?? 0} 条</div>
                      </div>
                      <div className="rounded-lg bg-slate-50 px-3 py-2">
                        <div className="font-medium text-slate-700">当前状态</div>
                        <div>{activePersonaStatus?.lastEvolutionPassed ? '最近一轮已通过' : '最近一轮未记录或未通过'}</div>
                        <div>{activeSubPersona?.description || '当前风格库未启用子人格'}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <label className="block">
                    ...
                    <input
                      type="number"
                      step={100}
                      placeholder="留空则不限定总字数"
                      value={taskOptions.desiredLength > 0 ? taskOptions.desiredLength : ''}
                      onChange={(event) =>
                        handleTaskOptionChange('desiredLength', event.target.value ? Number(event.target.value) || 0 : 0)
                      }
                      className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 outline-none focus:border-report-accent focus:ring-2 focus:ring-report-accent/20"
                    />
                  </label>
