...
const SOURCE_DIR = path.join(ROOT_DIR, "晚点LatePost");
...
async function main() {
  if (ACTIVE_PROFILE_ID !== "latepost") {
    throw new Error("请使用 --profile latepost 运行该脚本。");
  }
  ...
  if (!(await pathExists(SOURCE_DIR))) {
    throw new Error(`缺少 LatePost 原始目录：${SOURCE_DIR}`);
  }
  ...
  for (const monthEntry of monthDirs) {
    ...
    for (const articleEntry of articleDirs) {
      ...
      const content = normalizeContent(await readUtf8(contentPath));
      if (!content) {
        emptyContentCount += 1;
        manifest.push({
          source_dir: articleDirPath,
  ...
  await writeUtf8(
    path.join(METADATA_DIR, "latepost_import_manifest.jsonl"),
    manifest.map((item) => JSON.stringify(item)).join("\n") + "\n"
  );
  await writeJson(path.join(METADATA_DIR, "latepost_import_summary.json"), {
