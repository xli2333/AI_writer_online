...
const SOURCE_DIR = path.join(ROOT_DIR, "虎嗅APP");
...
async function main() {
  if (ACTIVE_PROFILE_ID !== "huxiu") {
    throw new Error("请使用 --profile huxiu 运行该脚本。");
  }
  ...
  if (!(await pathExists(SOURCE_DIR))) {
    throw new Error(`缺少虎嗅APP原始目录：${SOURCE_DIR}`);
  }
  ...
  await writeUtf8(
    path.join(METADATA_DIR, "huxiu_import_manifest.jsonl"),
    manifest.map((item) => JSON.stringify(item)).join("\n") + "\n"
  );
  await writeJson(path.join(METADATA_DIR, "huxiu_import_summary.json"), {
