...
const SOURCE_DIR = path.join(ROOT_DIR, "新智元");
...
async function main() {
  if (ACTIVE_PROFILE_ID !== "xinzhiyuan") {
    throw new Error("请使用 --profile xinzhiyuan 运行该脚本。");
  }
  ...
  if (!(await pathExists(SOURCE_DIR))) {
    throw new Error(`缺少新智元原始目录：${SOURCE_DIR}`);
  }
  ...
  await writeUtf8(
    path.join(METADATA_DIR, "xinzhiyuan_import_manifest.jsonl"),
    manifest.map((item) => JSON.stringify(item)).join("\n") + "\n"
  );
  await writeJson(path.join(METADATA_DIR, "xinzhiyuan_import_summary.json"), {
