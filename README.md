...
原因很直接：当前版本是纯前端 Vite 应用，但已经整理为 BYOK 入口模式。用户必须在浏览器里输入自己的 Gemini API Key，key 默认只保存在当前会话的 `sessionStorage` 中。因此它适合公开静态部署的 BYOK 工具，不适合平台统一代付费、统一配额控制或后端权限隔离场景。
...
当前默认采用 BYOK 模式，不依赖平台侧后端密钥。

- 用户在入口页输入自己的 Gemini API Key
- key 仅保存在当前浏览器会话的 `sessionStorage`
- 使用的 key 名：`GEMINI_API_KEY`
- 应用会在进入工作台前先做一次 key 校验

这意味着：

- 你可以把前端直接部署到 Vercel 这类静态托管平台
- 平台本身不需要保存你的 Gemini API Key
- 成本、配额和速率限制都由用户自己的 key 承担

这不等于“隐藏用户自己的 key”。  
BYOK 的含义是：用户在自己的浏览器里输入自己的 key，由前端直接调用 Gemini。  
如果你以后要做统一计费、平台代付费、配额控制或更高稳定性的 Deep Research，再迁移到服务端代理模式。
...
因为这是前端直连的 BYOK 模式，Gemini API Key 通过界面输入，并默认保存在当前浏览器会话的 `sessionStorage` 中，而不是通过 Vite 环境变量读取。
...
如果你现在的目标是尽快上线可用版本，当前仓库已经整理成适合公开部署的 BYOK 前端：

1. 部署到 Vercel
2. 不配置平台侧 `GEMINI_API_KEY`
3. 让用户在入口页填写自己的 Gemini API Key
4. 前端直接使用用户自己的 key 调用 Gemini

这种模式最适合：

- 个人工具
- 小范围团队内部使用
- 先验证工作流、后决定是否做后端化的产品阶段
...
5. Node.js：`20+`
6. Environment Variables：不配置平台侧 `GEMINI_API_KEY`

说明：

- 入口页会要求用户先填写自己的 Gemini API Key
- key 会先做一次轻量校验，再允许进入工作台
- key 默认只保存在当前标签页会话，不会随浏览器关闭长期保留
- 仓库中的 `raw_materials/` 和 `rag_assets/` 仍需要保留，它们是模板与参考能力的一部分
- 其余与部署无关的本地参考目录可通过 `.vercelignore` 排除
